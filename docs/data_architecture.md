# Data architecture — данные и сессии «Арены переговоров»

Единый документ по data design: таблицы, связи, контекст модели, история ходов,
срок хранения и очистка. Исполняемая SQL-схема остаётся в миграции
[0001_initial.sql](../backend/migrations/versions/0001_initial.sql), чтобы её мог применить Alembic.

Статус реализации: БД, миграции, очистка, авторизация и каталог backend реализованы.
Frontend остаётся инфраструктурной заглушкой. AI по умолчанию возвращает фиксированный mock-ответ;
внешний API-адаптер есть, но Context Builder, Evaluator, Game Engine и обработчик хода
ниже описывают проектное решение, а не готовую функциональность.
...
Реализованная схема: `backend/migrations/versions/0001_initial.sql` плюс миграция
`0002_user_password_hash.py`, добавляющая хранение хеша пароля. Применяйте `alembic upgrade head`,
чтобы получить актуальную схему. Этот документ описывает также будущую игровую логику;
Docker-каркас ещё не реализует обработчик ходов.
...
| `password_hash` | text, nullable | Хеш пароля с солью и параметрами алгоритма; исходный пароль не хранится |
| `display_name` | varchar(120) | Имя |
| `role` | varchar(20) | `player`, `admin` |
| `created_at` | timestamptz | Создание |
| `updated_at` | timestamptz | Изменение |

`password_hash` добавлен миграцией `0002`. Для существующих пользователей остаётся NULL:
миграция не придумывает пароль и не создаёт общих учётных данных. NULL означает, что пароль
ещё не задан и вход по паролю недоступен. Будущий обработчик регистрации обязан создавать
хеш до сохранения пользователя; проверка входа отклоняет NULL. Endpoint'ы регистрации
и входа добавлены в объединённой версии. Хеш не возвращают клиенту и не пишут в логи.

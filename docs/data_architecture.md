...
Статус реализации: БД, миграции, очистка, авторизация, каталог, игровой движок и редактор контента реализованы.
React-интерфейс работает с API. AI по умолчанию возвращает демонстрационные ответы;
для реального AI нужно настроить внешнего провайдера.
...
Реализованная схема: `backend/migrations/versions/0001_initial.sql` плюс миграции
`0002_user_password_hash.py` и `0003_user_login.py`. Применяйте `alembic upgrade head`,
чтобы получить актуальную схему. Часть описанных далее игровых механик остаётся проектным решением;
актуальные API-контракты можно проверить в Swagger.
...
| `login` | varchar(40), unique без учёта регистра | Логин для входа; у старых записей техническое значение `user-<uuid>` |
...
ещё не задан и вход по паролю недоступен. Регистрация создаёт хеш до сохранения пользователя;
проверка входа отклоняет NULL. `login` добавлен миграцией `0003`. Вход принимает логин или
email. Хеш не возвращают клиенту и не пишут в логи.

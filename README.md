...
| Часть | Состояние |
|---|---|
| БД | Миграции, ограничения, автоматическая очистка старых сессий и отдельное хранение пройденных сюжетных миссий |
| Backend | FastAPI: авторизация, каталог, игровые сессии, движок, ходы через WebSocket и защищённый редактор контента |
| Frontend | React-интерфейс: тренировки, сюжетные миссии, свой диалог, кабинет с историей и статистикой, сцена разговора в формате визуальной новеллы, база знаний, авторизация и админка |
| AI | Игровой движок с оценкой и ответом собеседника. По умолчанию `AI_PROVIDER=mock`: ответы заготовлены, оценка опирается на простые демонстрационные правила |
...
| migrate | Exited (0) | Успешно применил миграции 0001–0004 и завершился |
...
Ожидается **0004 (head)**. В users должны быть поля password_hash и login.
...
После удаления сессии исчезает её результат и запись разговора в кабинете. Успешно пройденные сюжетные миссии остаются в `story_mission_progress`, поэтому следующие сцены не закрываются повторно.

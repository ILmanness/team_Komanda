...
| Backend | Каркас FastAPI с healthcheck и подключением к БД; регистрации, входа и игровых API пока нет |
...
В нём хранится хеш, не исходный пароль. Наличие колонки не означает готовую авторизацию.
...
~~~sh
git clone --branch ms/update-reademe-21.09.2026 https://github.com/ILmanness/team_Komanda.git
cd team_Komanda
~~~
...
~~~sh
git fetch origin
git switch ms/update-reademe-21.09.2026
git pull --ff-only
~~~

**Все дальнейшие команды выполняются из корня проекта, где лежит compose.yaml.**
Новая миграция и эта инструкция находятся в указанной ветке. Main автоматически не обновляется.
...
Будущая реализация входа должна отклонять вход по паролю для таких записей.
...
стойкого алгоритма хеширования паролей с солью. Регистрация и вход ещё не реализованы.
...
реквизиты с правами миграции, авторизации нет. Для публичного сервера нужны
авторизация, TLS, разграничение прав и production-настройки.
